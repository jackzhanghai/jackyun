...
{
    id propertyID = (id) ABRecordCopyValue(record, anID);
    if (!propertyID) propertyID = @"";
	return [propertyID autorelease];
}
...
#pragma mark Contact Name PCUtility
...
- (NSArray *) labelsForProperty: (ABPropertyID) anID
{
	...
	for (int i = 0; i < ABMultiValueGetCount(theProperty); i++)
	{
        ...
        //Add by kuit
        //if (!label) label = [[NSString alloc] initWithString:@""];
...
{
    NSMutableArray *items = [NSMutableArray array];
    ABMultiValueRef values = ABRecordCopyValue([self record], aProperty);
    for (int k = 0; k < ABMultiValueGetCount(values); k++) {
        NSString * label = (NSString*)ABAddressBookCopyLocalizedLabel(ABMultiValueCopyLabelAtIndex(values, k));
        NSString * value = (NSString*)ABMultiValueCopyValueAtIndex(values, k);
        
        NSMutableDictionary *md = [NSMutableDictionary dictionary];
		[md setObject:value forKey:@"value"];
...
//Add by kuit
- (void) setCreationDate: (NSDate *) aDate {[self setDate: aDate forProperty: kABPersonCreationDateProperty];}
- (void) setModificationDate: (NSDate *) aDate {[self setDate: aDate forProperty: kABPersonModificationDateProperty];}
...
//用于数组里遍历比较两Contact对象是否相同
- (BOOL)isEqual:(id)anObject
{
    return self.recordID == ((ABContact *)anObject).recordID;
}
