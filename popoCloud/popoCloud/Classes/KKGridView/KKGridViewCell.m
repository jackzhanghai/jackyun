...
- (id)initWithFrame:(CGRect)frame reuseIdentifier:(NSString *)reuseIdentifier
{
    if ((self = [super initWithFrame:frame])) {
        ...
        _imageView.backgroundColor = [UIColor colorWithRed:227.0/256 green:230.0/256 blue:235.0/256 alpha:1];
		[self.contentView addSubview:_imageView];
        
        _selectedImageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.bounds.size.width - 20, 4, 16, 16)];
        _selectedImageView.image = [UIImage imageNamed:@"checkbox"];
...
- (void)changeSelectImage:(BOOL)selected
{
    if (selected)
    {
        _selectedImageView.image = [UIImage imageNamed:@"checkbox_d"];
    }
    else
    {
        _selectedImageView.image = [UIImage imageNamed:@"checkbox"];
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated
{
    ...
    self.selectedImageView.hidden = !editing;
