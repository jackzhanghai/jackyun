...
- (void)dealloc 
{
   [imageView_ release], imageView_ = nil;
    ...
    if (self.currentCache) {
        [self.currentCache cancel];
        self.currentCache.delegate = nil;
...
- (id)initWithFrame:(CGRect)frame
{
   ...
   if (self) {
      ...
      //[self setImage:[UIImage imageNamed:@"picture.png"]];
       self.labelProgress = [[[UILabel alloc] initWithFrame:CGRectMake(20, 200, 280, 100)]  autorelease];
       self.labelProgress.adjustsFontSizeToFitWidth = YES;
       self.labelProgress.center = self.center;
       self.labelProgress.backgroundColor = [UIColor clearColor];
       self.labelProgress.textColor= [UIColor redColor];
       self.labelProgress.textAlignment = UITextAlignmentCenter;
       [self addSubview:self.labelProgress];
       
       UIActivityIndicatorView *loadingIndictator = [[UIActivityIndicatorView alloc]
                                                  initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
       loadingIndictator.color = [UIColor grayColor];
       [self addSubview:loadingIndictator];
       self.indicator = loadingIndictator;
       self.indicator.center = self.center;
       [loadingIndictator release];
   }
   return self;
}
...
{
    CGSize imgViewSize = imageView_.bounds.size;
    //CGFloat deviceScale =  [[UIScreen mainScreen] scale];
    //不考虑高清效果，和图片集一致，尽量撑满屏幕
    CGFloat deviceScale =  1;
    if ( (newSize.width< imgViewSize.width*deviceScale)
        &&
        (newSize.height< imgViewSize.height*deviceScale)) {
...
{
//    if (newImage) {
//        [self setContentModeForImgSize:newImage.size];
//    }
    if (loadImageFailed) {
...
//    if ( CGRectEqualToRect([self bounds], [imageView_ frame]) == NO) {
//        [imageView_ setFrame:[self bounds]];
//        
//        [self setContentModeForImgSize:imageView_.image.size];
//    }

    CGPoint p = imageView_.center;
    CGPoint labelProgressCentor=  CGPointMake(p.x, p.y+30);
    self.labelProgress.center = labelProgressCentor;
...
- (CGFloat)scaleToRestoreAfterRotation
{
  ...
  //CGFloat contentScale = self.zoomScale;
   CGFloat contentScale = 1;
   NSLog(@"Current minimumZoomScale  %f",self.minimumZoomScale);
