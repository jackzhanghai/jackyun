...
- (id)initWithFrame:(CGRect)frame
{
   ...
   if (self) {
      ...
      thumbSize_ = CGSizeMake(THUMBNAIL_SIZE, THUMBNAIL_SIZE);
...
- (KTThumbView *)dequeueReusableThumbView
{
   ...
   if (thumbView != nil) {
      ...
      // thumbView.image = [UIImage imageNamed:@"picture.png"];
       thumbView.image = nil;
...
- (void)layoutSubviews 
{
    ...
    int thumbCount = dataSource_ ? [dataSource_ thumbsViewNumberOfThumbs:self] : 0;
...
//    DLogNotice(@"contentSize=%@",NSStringFromCGSize(contentSize));
   
    //此处rowsPerView的计算为了使ipad上竖屏时多一行的显示，所以加了1，根据缩略图高度会有调整，也可能不需要加1
    NSInteger rowsPerView = roundf(visibleHeight / rowHeight) + (visibleWidth == 768 ? 1 : 0);
...
//    DLogNotice(@"rowsPerView=%d,topRow=%d,bottomRow=%d",rowsPerView,topRow,bottomRow);
//    DLogNotice(@"startAtIndex=%d,stopAtIndex=%d",startAtIndex,stopAtIndex);
    ...
    for (int index = startAtIndex; index < stopAtIndex; index++) {
        ...
        if (isThumbViewMissing) {
            ...
            thumbView.frame = CGRectMake(x, y, thumbSize_.width, thumbSize_.height);

            // Store the current index so the thumb view can 
            // find it later.
//            thumbView.tag = index;//该句移到[dataSource_ thumbsView:self thumbForIndex:index]执行了
