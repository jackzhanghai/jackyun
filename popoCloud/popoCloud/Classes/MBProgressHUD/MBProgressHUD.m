...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated afterDelay:(NSTimeInterval)delay {
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    [hud setGraceTime:delay];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated  WithTag:(int)viewTag{
	MBProgressHUD *hud = [MBProgressHUD HUDForView:view  WithTag:viewTag];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (MBProgressHUD *)HUDForView:(UIView *)view  WithTag:(int)viewTag {
	MBProgressHUD *hud = nil;
	NSArray *subviews = view.subviews;
	Class hudClass = [MBProgressHUD class];
	for (UIView *aView in subviews) {
		if ([aView isKindOfClass:hudClass]) {
            if (((MBProgressHUD *)aView).tag == viewTag) {
                hud = (MBProgressHUD *)aView;
            }			
		}
	}
