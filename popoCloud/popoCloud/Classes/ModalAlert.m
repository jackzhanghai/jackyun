...
+ (BOOL) confirm: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = ([ModalAlert ask:statement withCancel:NSLocalizedString(@"Cancel", nil) withButtons:[NSArray arrayWithObject:NSLocalizedString(@"OK", nil)]] != 0);
	[statement release];
	return	answer;
}

+(NSString *) textQueryWith: (NSString *)question prompt: (NSString *)prompt text:(NSString*)text button1: (NSString *)button1 button2:(NSString *) button2
{
	// Create alert
	CFRunLoopRef currentLoop = CFRunLoopGetCurrent();
	ModalAlertDelegate *madelegate = [[ModalAlertDelegate alloc] initWithRunLoop:currentLoop];
	...
	if (prompt) tf.placeholder = prompt;
...
{
	return [ModalAlert textQueryWith:question prompt:prompt text:nil button1:NSLocalizedString(@"Cancel", nil) button2:NSLocalizedString(@"OK", nil)];
}

+ (NSString *) ask: (NSString *) question withText: (NSString *) text
{
	return [ModalAlert textQueryWith:question prompt:nil text:text button1:NSLocalizedString(@"Cancel", nil) button2:NSLocalizedString(@"OK", nil)];
}
...
+ (BOOL)confirm:(NSString *)question prompt:(NSString *)prompt
{
	...
	// Create Alert
	ModalAlertDelegate *madelegate = [[ModalAlertDelegate alloc] initWithRunLoop:currentLoop];
	UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:question message:prompt delegate:madelegate cancelButtonTitle:NSLocalizedString(@"Cancel", nil) otherButtonTitles:NSLocalizedString(@"OK", nil), nil];
	[alertView show];
	...
	// Retrieve answer
	BOOL answer = madelegate.index != alertView.cancelButtonIndex;
	[alertView release];
	[madelegate release];
