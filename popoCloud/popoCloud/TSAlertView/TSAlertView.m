...
- (void) dismissWithClickedButtonIndex: (NSInteger)buttonIndex animated: (BOOL) animated
{
	...
	if ( [self.delegate respondsToSelector: @selector(tsAlertView:willDismissWithButtonIndex:)] )
	{
		[self.delegate tsAlertView: self willDismissWithButtonIndex: buttonIndex ];
	}
...
- (void) releaseWindow: (int) buttonIndex
{
	if ( [self.delegate respondsToSelector: @selector(tsAlertView:didDismissWithButtonIndex:)] )
	{
		[self.delegate tsAlertView: self didDismissWithButtonIndex: buttonIndex ];
	}
...
- (void) onButtonPress: (id) sender
{
	...
	{
		if ( [self.delegate respondsToSelector: @selector(tsAlertViewCancel:)] )
		{
			[self.delegate tsAlertViewCancel: self ];
		}	
	}
